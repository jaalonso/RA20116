...
value "(2::nat) * 3" 
(* \<leadsto> "Suc (Suc (Suc (Suc (Suc (Suc 0)))))" :: "nat"*)
...
text {* Los numerales están sobrecargados. Por ejemplo, el 1 puede ser
  ...
  conectivas (\<not>, \<and>, \<or>, \<longrightarrow> y \<leftrightarrow>) y los cuantificadores (\<forall> y \<exists>). 
...
value "x#(y#(z#[]))" 
(* \<leadsto> "[x, y, z]" :: "'a list" *)

value "(1::int)#(2#(3#[]))"
(* \<leadsto> "[1, 2, 3]" :: "int list" *)
...
value "let xs = [a,b,c] in hd xs = a \<and> tl xs = [b,c]" 
(* \<leadsto> "True" :: "bool" *)
...
value "(\<lambda>x. x + x) 1::int" 
(* \<leadsto> "2" :: "int" *)

section {* Condicionales *}

text {* El valor absoluto del entero x es x, si "x \<ge> 0" y es -x en caso 
  contrario. *}
...
(* \<leadsto> Lista.Cons a (Lista.Cons b (Lista.Cons c Vacia)) *)
...
value "conc (Cons a (Cons b Vacia)) (Cons c Vacia)"
(* \<leadsto> Cons a (Cons b (Cons c Vacia) *)
...
value "suma 3" 
(* \<leadsto> Suc (Suc (Suc (Suc (Suc (Suc 0)))) *)
...
value "sumaImpares 3" 
(* \<leadsto> Suc (Suc (Suc (Suc (Suc (Suc (Suc (Suc (Suc 0)))))))) *)
